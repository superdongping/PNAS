function c_pos_sum=Mice_trajectory(movie_input_filename,start_time,duration_time,group_no)
...
I_background=read(E_M_movie,192);
...
trace_fig_name=['trace_',filename(1:end-4),num2str(group_no)];
...
figure(03)
plot(aver_vel)
vel_fig_name=['vel_',filename(1:end-4),num2str(group_no)];
saveas(gcf,vel_fig_name,'tif')
vel_file_name=['Speed_',filename(1:end-4),num2str(group_no),'.xls'];
